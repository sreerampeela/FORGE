...
class MultiDrugFORGE:
    '''
    Main class object of the Multi Drug FORGE framework.
...
2. Gene dependency scores (with gene of interest), and
3. Drug IC50 profiles of all the drugs targeting the single gene of interest
...
or hyper-parameter noise.
    ...
    def __init__(self, exp_path, dependency_path, ic50_path, drug_target_data, target_name, log_file, overwrite,
                 drug_names=None):
        '''
    ...
    # -------------------------------------------------------------------------
        ...
        if drug_names is None:
            drug_target_df = pd.read_csv(drug_target_data, header=0)
            drugs_of_interest = []
            for _, row in drug_target_df.iterrows():
                if target_name in row['Target'].split(','):
                    drugs_of_interest.append(row['Drug'])
            drugs_of_interest = list(set(drugs_of_interest))
        else:
            drugs_of_interest = drug_names
        print(f'Total number of drugs targeting {target_name}:', len(
            drugs_of_interest))
        drugs_of_interest = list(
            set(drugs_of_interest) & set(ic50_data.columns))
        if not drugs_of_interest:
            raise ValueError('None of the drugs have IC50 values.')
        else:
            print(f'Drugs to be tested: {drugs_of_interest}')

        # clean the data for drug and target-specific entries
        logging.info('Cleaning the input datasets..')
        ic50_data = ic50_data[drugs_of_interest].dropna()
        ...
        if not common_cell_lines:
            raise ValueError(
                f'No cell lines were common for the drugs and {target_name}. Exiting!')

        else:
            logging.info(
                f'Identified a total of {len(common_cell_lines)} samples in input data')
            if len(common_cell_lines) < 200:
                logging.warning(
                    f'Too few cell lines common for drugs and target. Model may be suboptimal!')
                print(
                    'Warning: Too few cell lines common for drugs and target. Model may be suboptimal!')
...
# -------------------------------------------------------
        ...
        self.mean_ic50 = self.ic50_data.iloc[train_pos_full].mean(
            axis=0).values  # Vector of means
        # -------------------------------------------------------
    ...
    def train_instance(self, G_train, D_train, I_train,
        ...
        hI = np.asarray(hI).reshape(-1, h_dims)
        with threadpool_limits(limits=2, user_api='blas'):
            ...
            # logging.info('Error vectors dimensions:', err_d.shape, err_i.shape)
                ...
                train_err_i = np.asarray(train_err_i).reshape(-1, h_dims)
            # logging.info('Error vectors dimensions after reshaping:',
            #   err_d.shape, err_i.shape)
                # compute d and i mean losses
                mse_d = np.mean(train_err_d**2)
                # array of length h_dims
                mse_i_per_drug = np.mean(train_err_i**2, axis=0)
                # scalar average
                mse_i_avg = np.mean(mse_i_per_drug)

            # Total Loss (Balanced 50/50 between Dependency task and Drug task)
                train_loss = 0.5 * (mse_d + mse_i_avg)
    ...
    def validate_instance(self, G_val, D_val, I_val, W, hD, hI):
        ...
        # val_losses = []
        # val_losses = []
        hdims = len(self.drugs)
        G_val = np.asarray(G_val)       # shape (n_samples, n_genes)
        D_val = np.array(D_val).reshape(-1, 1)
        I_val = np.array(I_val).reshape(-1, hdims)
        hD = np.asarray(hD).reshape(-1, 1)     # shape (k, 1) or (k,)
        hI = np.asarray(hI).reshape(-1, hdims)
        Z = G_val @ W
        ...
        err_i = np.asarray(err_i).reshape(-1, hdims)
        # logging.info('Error vectors dimensions after reshaping:',
        #   err_d.shape, err_i.shape)
        mean_d_val = np.mean(err_d**2)
        mse_i_per_drug_val = np.mean(
            err_i**2, axis=0)  # array of length h_dims
        # scalar average
        mse_i_avg_val = np.mean(mse_i_per_drug_val)

        # Total Loss (Balanced 50/50 between Dependency task and Drug task)
        val_loss = 0.5 * (mean_d_val + mse_i_avg_val)
        ...
        # --------------------------
            with threadpool_limits(limits=2, user_api='blas'):
                for idx, fold_idx in enumerate(self.training_cv_splits):
                    ...
                    hI = np.random.randn(K, len(self.drugs))
                    ...
                    D_train = self.dep_data[train_cellLines].values.reshape(
                        -1, 1)
                    D_val = self.dep_data[val_cellLines].values.reshape(
                        -1, 1)
                    D_train = D_train - self.mean_dep
                    D_val = D_val - self.mean_dep
                    I_train = self.ic50_data.loc[train_cellLines].values.reshape(
                        -1, hdims)
                    I_val = self.ic50_data.loc[val_cellLines].values.reshape(
                        -1, hdims)
    ...
    # Create study
    # --------------------------
        instance_name = f'{self.target}_mdFORGE_optuna'
    ...
    def train_forge(self, optuna_trials=100, quiet=False,
        ...
        D_train = D_train - self.mean_dep
        D_val = D_val - self.mean_dep
        I_train = self.ic50_data.loc[train_cellLines].values.reshape(-1, hdims)
        I_val = self.ic50_data.loc[val_cellLines].values.reshape(-1, hdims)
        I_train = I_train - self.mean_ic50
        I_val = I_val - self.mean_ic50
        W = np.random.randn(G_train.shape[1],
                            num_latent)  # gxk matrix
        hD = np.random.randn(num_latent, 1)
        hI = np.random.randn(num_latent, hdims)
    ...
    def save_forge(self, path=None):
        if path is None:
            model_name = f"{self.target}_multidrug_forge.pkl"
        else:
            model_name = path
    ...
    def run_Pipeline(self, n_splits, seed_val, tuning_epochs=500, quiet=False,
      ...
      5. Save the full object
        ...
        logging.info(
            "============================================================")
        logging.info(
            f"Running multi drug FORGE Pipeline for {self.target}")
        logging.info(
            "============================================================")
        ...
        for drug in self.drugs:
            spearman_ic50 = spearman_corr_fun(
                exp_data=exp_data_train, y=ic50_train[drug])
            top_hcgs_subset = spearman_ic50.abs().sort_values(
                ascending=False).head(n_hcgs).index.tolist()
            top_drug_hcgs.extend(top_hcgs_subset)
        top_hcgs_ic50 = list(set(top_drug_hcgs))
        logging.info(
        ...
        end_time1 = time.time()
        logging.info(
            f'FORGE training completed in {end_time1 - start_time1} seconds..Run test later for each drug..')
        # test_ids = self.test_cellLines
        # G_test = self.exp_hcg.loc[test_ids]
        # G_test = ((G_test - self.mean_exp) / self.std_exp).values
        # # 1D vector
        # D_test = self.dep_data.loc[test_ids].values.reshape(
        #     -1, 1)
        # # 1D vector
        # I_test = self.ic50_data.loc[test_ids].values.reshape(
        #     -1, len(self.drugs))
        ...
        logging.info(
            f'Pipeline successfully completed for multi drug FORGE for {self.target}. Saving FORGE model..')
