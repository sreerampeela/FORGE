...
or hyper-parameter noise.
    ...
    def __init__(self, exp_path, dependency_path, ic50_path, drug_name, target_name):
        '''
...
# -------------------------------------------------------
    ...
    # --------------------------
        ...
        # --------------------------
            ...
            self.__setattr__(f'trial{trial.number}_val_loss', mean_val_loss)
    ...
    def train_forge(self, optuna_trials=100, quiet=False,
        ...
        for idx, fold_idx in enumerate(self.training_cv_splits):
            # Initialize W, hD, hI separately for each fold
            W = np.random.randn(len(self.hcg_list),
            ...
            G_train = self.exp_hcg.loc[train_cellLines,
                                       self.hcg_list].values
            G_val = self.exp_hcg.loc[val_cellLines, self.hcg_list].values
            D_train = self.dep_data[train_cellLines].values.reshape(-1, 1)
            D_val = self.dep_data[val_cellLines].values.reshape(-1, 1)
            I_train = self.ic50_data[train_cellLines].values.reshape(-1, 1)
            I_val = self.ic50_data[val_cellLines].values.reshape(-1, 1)
            ...
            self.__setattr__(f'fold{str(idx)}_epoch_train_loss', train_losses)
            self.__setattr__(
                f'fold{str(idx)}_epoch_val_loss', val_losses)
            self.__setattr__(f'fold{str(idx)}_W', tuned_W)
            self.__setattr__(f'fold{str(idx)}_hD', tuned_hD)
            self.__setattr__(f'fold{str(idx)}_hI', tuned_hI)
            ...
            logging.info(
                f"Final training loss for fold {str(idx)}: {tuned_train_loss:.5f}. "
                f"Validation loss for fold {str(idx)}: {tuned_val_loss:.5f}"
            )
    ...
    def run_Pipeline(self, n_splits, seed_val, tuning_epochs=500, quiet=False,
      ...
      5. Save the full object
        ...
        logging.info(
            f'Cutoffs for IC50: {spearman_ic50.loc[top_hcgs_ic50[-1]]} and dependency: {spearman_ic50.loc[top_hcgs_dep[-1]]}')
        ...
        G_test = self.exp_hcg.loc[test_ids].values
